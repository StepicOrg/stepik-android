...
constructor(
    ...
    fun fetchDownloadedCourses() {
        compositeDisposable += downloadsInteractor.fetchDownloadItems()
            ...
            .subscribeBy(
                onNext = { state = downloadItemsStateMapper.replaceDownloadItem(state, it) },
    ...
    fun removeCourseDownload(course: Course) {
        ...
        compositeDisposable += courseDownloadInteractor
            ...
            .subscribeBy(
                onComplete = { state = downloadItemsStateMapper.replaceDownloadItem(state, DownloadItem(course, DownloadProgress.Status.NotCached)) },
