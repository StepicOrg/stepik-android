        ...
        if (action != null && action == NotificationActionsHelper.ISSUED_CERTIFICATE) {
            ...
            val intent =
                if (courseId >= 0 && modulePosition != null && modulePosition >= 0) {
                    CourseActivity.createIntent(applicationContext, courseId, tab = CourseScreenTab.SYLLABUS, source = CourseSourceAnalyticData.Notification)
                } else {
                    CourseActivity.createIntent(applicationContext, relatedCourse, tab = CourseScreenTab.SYLLABUS, source = CourseSourceAnalyticData.Notification)
                }
    ...
    private fun openLearnNotification(context: Context, notification: Notification): Boolean {
        if (notification.action != null && notification.action == NotificationActionsHelper.ISSUED_CERTIFICATE) {
            ...
            if (courseId != null && courseId >= 0 && modulePosition != null && modulePosition >= 0) {
                val intent = CourseActivity.createIntent(context, courseId, tab = CourseScreenTab.SYLLABUS, source = CourseSourceAnalyticData.Notification) // Intent(applicationContext, SectionActivity::class.java)
