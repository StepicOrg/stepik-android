...
public class DatabaseFacade {
    ...
    @Inject
    IDao<Course> mCoursesEnrolledDao;
    @Inject
    IDao<Course> mCoursesFeaturedDao;

    public DatabaseFacade() {
        MainApplication.component().inject(this);
        mCoursesEnrolledDao.setTableName(Table.enrolled.getStoreName());
        mCoursesFeaturedDao.setTableName(Table.featured.getStoreName());
    }
    ...
    public IDao<Course> getCourseDao(Table table) {
        if (table == Table.featured) {
            return mCoursesFeaturedDao;
        } else {
            return mCoursesEnrolledDao;
        }
    }
    ...
    public Course getCourseById(long courseId, Table type) {
        return getCourseDao(type).get(DBStructureCourses.Column.COURSE_ID, courseId + "");
    }
    ...
    public void updateOnlyCachedLoadingCourse(Course course, Table type) {
        ...
        getCourseDao(type).update(DBStructureCourses.Column.COURSE_ID, course.getCourseId() + "", cv);
    }

    public List<Course> getAllCourses(Table type) {
        return getCourseDao(type).getAll();
    }

    public void addCourse(Course course, Table type) {
        getCourseDao(type).insertOrUpdate(course);
    }

    public void deleteCourse(Course course, Table type) {
        getCourseDao(type).delete(DBStructureCourses.Column.COURSE_ID, course.getCourseId() + "");
    }
    ...
    public void clearCacheCourses(DatabaseFacade.Table type) {
