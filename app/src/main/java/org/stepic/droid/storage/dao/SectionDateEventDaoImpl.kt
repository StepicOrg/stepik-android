...
constructor(
    ...
    override fun parsePersistentObject(cursor: Cursor): SectionDateEvent {
        val eventIdIndex = cursor.getColumnIndex(DbStructureSectionDateEvent.Columns.EVENT_ID)
        val sectionIdIndex = cursor.getColumnIndex(DbStructureSectionDateEvent.Columns.SECTION_ID)

        return SectionDateEvent(
                eventId = cursor.getLong(eventIdIndex),
                sectionId = cursor.getLong(sectionIdIndex)
        )
    }
