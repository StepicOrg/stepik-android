...
constructor(
    ...
    private fun getTimeToCompleteForSection(section: Section): Observable<Pair<Long, Long>> =
            api.getUnitsRx(section.units).flatMap {
                val ids = it.units?.map { it.lesson }?.toLongArray()
                api.getLessonsRx(ids)
            }.map {
                section.id to it.lessons!!.map { if (it.timeToComplete == 0L) it.steps.size * DEFAULT_STEP_LENGTH_IN_SECONDS else it.timeToComplete }.sum()
            }.toObservable()
