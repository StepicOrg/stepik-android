package org.stepik.android.ktlint
...
object Main {
    ...
    @Option(names = ["--rules-to-ignore"], description = ["Comma separated list of rules to ignore"])
    private var rulesToIgnore: String? = null
    ...
    fun main(args: Array<String>) {
        ...
        val ignoredRules = rulesToIgnore
            ?.split(',')
            ?: emptyList()
        ...
        val ruleSetProviders = ServiceLoader.load(RuleSetProvider::class.java)
            .map { provider ->
                LazyRuleSetProvider(provider, ignoredRules)
            }
            .map { it.get().id to it }
    ...
    class LazyRuleSetProvider(
        private val ruleSetProvider: RuleSetProvider,
        private val ignoredRules: Iterable<String>
    ) : RuleSetProvider {
        override fun get(): RuleSet =
            ruleSetProvider.get().let { ruleSet ->
                RuleSet(ruleSet.id, *ruleSet.rules.filterNot { it.id in ignoredRules }.toTypedArray())
            }
    }
}